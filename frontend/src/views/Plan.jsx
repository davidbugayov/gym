...
export default function Plan() {
  ...
  const [viewMode, setViewMode] = useState('schedule') // 'schedule' | 'calendar'
  ...
  return <>
    <div className="hdr">
      <div>
        <h1>{t('Plan')}</h1>
        <div className="sub">{viewMode === 'calendar' ? t('Workout calendar & history') : t('Your weekly routine')}</div>
      </div>
    ...
    <div style={{ marginBottom: 16 }}>
      <Segmented
        options={[
          { value: 'schedule', label: t('Schedule'), icon: 'list' },
    ...
    {viewMode === 'calendar' ? (
      <PlanCalendar onSwitchToSchedule={() => setViewMode('schedule')} />
    ) : (
