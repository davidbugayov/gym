...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR Gymly — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { gymly_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.gymly_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an Gymly plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with Gymly'))} · gymly.duarte-santos.ch</footer>
</div></body></html>`
}
