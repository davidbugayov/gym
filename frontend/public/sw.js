/* Gymly service worker — runtime caching (works with Vite's hashed asset names).
   Media (img/gif) cache-first; everything else network-first with offline fallback. */
const CACHE = 'gymly-rt-v1'
...
self.addEventListener('push', e => {
  ...
  e.waitUntil(self.registration.showNotification(data.title || 'Gymly', {
    body: data.body || '',
    icon: 'icon-512.png',
    badge: 'icon-180.png',
    tag: data.tag || 'gymly',
