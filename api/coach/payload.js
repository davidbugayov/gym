...
function cleanWorkout(w) {
  return {
    ...
    ...(Number(w.calories) > 0 ? { calories: Math.round(Number(w.calories)) } : {}),
    ...(Number(w.distanceKm) > 0 ? { distanceKm: Math.round(Number(w.distanceKm) * 100) / 100 } : {}),
    ...(w.importedFrom === 'Google Health' ? { source: 'Google Health', activityType: w.healthExerciseType || null } : {}),
