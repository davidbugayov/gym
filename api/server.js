/* gymly-api — passkey (WebAuthn) auth + per-user state storage for Gymly
...
const RP_NAME = process.env.RP_NAME || 'Gymly';
...
const routes = {
  ...
  'POST /api/push/test': async (req, res) => {
    const user = readSession(req);
    if (!user) return json(res, 401, { error: 'not signed in' });
    await sendPush(user.id, { title: 'Gymly', body: 'Test notification ✅ — this is what alerts look like.', tag: 'test' });
    json(res, 200, { ok: true });
  },
